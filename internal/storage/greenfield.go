...
func (s *GnfdStorage) put(ctx *GnfdStorageContext, key string, value []byte, isOverWrite bool) error {
	// fmt.Println("RepoName: ", s.GetBucketName(), " key: ", key, "isOverwrite: ", isOverWrite)
	if ctx.Sem.TryAcquire(1) {
		ctx.Wg.Add(1)
		go func() {
			err := s.syncPut(ctx, key, value, isOverWrite)
			if err != nil {
				slog.Error("async put object failed.", "error", err, "key", key)
				panic(err)
			}
			ctx.Wg.Done()
		}()
...
func (s *GnfdStorage) syncPut(ctx *GnfdStorageContext, key string, value []byte, isOverWrite bool) error {
