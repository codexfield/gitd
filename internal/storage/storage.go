...
type GnfdStorage struct {
	ctx        *GnfdStorageContext
	GnfdClient client.IClient
	RepoName   string
	Account    *types.Account
}

func NewStorage(ctx *GnfdStorageContext, chainID, rpcAddress, privateKey, bucketName string) (*GnfdStorage, error) {
...
func (s *GnfdStorage) SetEncodedObject(obj plumbing.EncodedObject) (plumbing.Hash, error) {
	...
	buffer.Write(c)
	err = s.put(s.ctx, buildObjectsKey(obj.Hash()), buffer.Bytes(), false)
	return obj.Hash(), err
}
...
func (s *GnfdStorage) SetReference(reference *plumbing.Reference) error {
	...
	return s.put(s.ctx, buildReferenceKey(reference.Name()), val, true)
}
...
func (s *GnfdStorage) SetConfig(c *config.Config) error {
	...
	return s.put(s.ctx, ConfigKey, jsonBytes, true)
